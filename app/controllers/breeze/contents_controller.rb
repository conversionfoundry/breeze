    ...
    def add_editing_controls
      if request.format.try :html?
        ...
        if admin_signed_in? && !request.xhr? && view && view.respond_to?(:editor_html)
          response.body = response.body.sub(/(<body[^>]*>)/, "\\0<div class='editor-panel'><div class='editor-help'><h3>Editor Panel</h3><p>Select a page element or create a new one.</p></div></div><div class='website-panel'>")

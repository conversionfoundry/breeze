module Breeze
  module Admin
    class ContentsController < AdminController
      ...
      def create
        # Hack to store values for date params
        params[:content].select { |key, value| key.to_s.match(/1i\)/) }.each do | key, value|
          param_name = key[0..-5]
          date = Date.new params[:content]["#{param_name}(1i)"].to_i, params[:content]["#{param_name}(2i)"].to_i, params[:content]["#{param_name}(3i)"].to_i
          params[:content][param_name] = date
        end

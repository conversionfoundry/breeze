...
renumber_fields = (container) ->
  $(".custom-field", container).each (i) ->
    $("label", this).each ->
      $(this).attr "for", $(this).attr("for").replace(/^content_type_content_fields_attributes_[0-9]+/, "content_type_content_fields_attributes_" + i)

    $("input, select, textarea", this).each ->
      $(this).attr "id", $(this).attr("id").replace(/^content_type_content_fields_attributes_[0-9]+/, "content_type_content_fields_attributes_" + i)
      $(this).attr "name", $(this).attr("name").replace(/^content_type\[content_fields_attributes\]\[[0-9]+/, "content_type[content_fields_attributes][" + i)

    ...
    def breeze_form( name='Unnamed form', options={}, &block)
      ...
      form_tag form_results_path, options do |form|
        content = [].tap do |content|
          content << hidden_field_tag("[meta][form_name]", name)
          content << hidden_field_tag("[meta][success_function]", options[:success_function]) if options[:success_function]
          content << hidden_field_tag("[meta][recipient_email_field]", options[:recipient_email_field]) if options[:recipient_email_field]
          content << hidden_field_tag("[meta][sender_email_field]", options[:sender_email_field]) if options[:sender_email_field]
          content << hidden_field_tag("[meta][replyto_email_field]", options[:replyto_email_field]) if options[:replyto_email_field]
